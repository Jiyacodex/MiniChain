...
"""
        ...
        tx_call = Transaction(self.pk, contract_addr, 0, 1, gas_limit=1000, fee_per_gas=1, data="increment")
    ...
    def test_deploy_insufficient_balance(self):
        ...
        tx = Transaction(poor_pk, None, 1000, 0, gas_limit=50000, fee_per_gas=1, data=code)
        tx.sign(poor_sk)
    ...
    def test_call_non_existent_contract(self):
        ...
        tx = Transaction(self.pk, fake_receiver, 0, 0, gas_limit=50000, fee_per_gas=1, data="increment")
        tx.sign(self.sk)
...
"""
        ...
        tx_call = Transaction(self.pk, contract_addr, 0, 1, gas_limit=1000, fee_per_gas=1, data="anything")
        tx_call.sign(self.sk)
    ...
    def test_redeploy_same_address(self):
        ...
        # First deploy
        tx1 = Transaction(self.pk, None, 0, 0, gas_limit=50000, fee_per_gas=1, data=code)
        tx1.sign(self.sk)
        ...
        # Attempt redeploy
        tx2 = Transaction(self.pk, None, 0, next_nonce, gas_limit=50000, fee_per_gas=1, data=code)
        tx2.sign(self.sk)
    ...
    def test_balance_and_nonce_updates(self):
        ...
        tx_deploy = Transaction(self.pk, None, 10, initial_nonce, gas_limit=50000, fee_per_gas=1, data=code)
        tx_deploy.sign(self.sk)
