...
"""
        deploy_tx = self._sign(Transaction(self.sender_pk, None, amount=100, nonce=0, data=code, gas_limit=50000, fee_per_gas=1))
...
"""
        deploy_tx = self._sign(Transaction(self.sender_pk, None, amount=0, nonce=0, data=code, gas_limit=50000, fee_per_gas=1))
        receipt = self.state.apply_transaction(deploy_tx)
        self.assertEqual(receipt.status, 1)
        contract_addr = receipt.contract_address
