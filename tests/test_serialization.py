...
def test_block_serialization_determinism():
   ...
   # Add the miner field
    block1 = Block(index=1, previous_hash="0"*64, transactions=[tx1], target=int("F"*64, 16), timestamp=999999, miner="a" * 40)
    block2 = Block(index=1, previous_hash="0"*64, transactions=[tx2], target=int("F"*64, 16), timestamp=999999, miner="a" * 40)
...
def test_block_from_dict_rejects_tampered_payload():
    ...
    block = Block(
        ...
        target=int("F"*64, 16), timestamp=999999, miner="a"*40
    )
